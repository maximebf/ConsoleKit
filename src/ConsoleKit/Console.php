...
class Console implements TextWriter {
   ...
   */
  function __construct(array $commands = array(), OptionsParser $parser = null, TextWriter $writer = null) {
   ...
   * @return Console
   */
  function setOptionsParser(OptionsParser $parser) {
   ...
   * @return Console
   */
  function setExitOnException($exit = true) {
   ...
   */
  function setVerboseException($enable = true) {
   ...
   * Registers a command
   *
   * @param \Closure(array $args, array $options = [], Console $console) $callback Associated class name, function name, Command instance or closure
   * @param string $alias Command name to be used in the shell
   * @param bool $default True to set the command as the default one
   * @return Console
   */
  function addCommand($callback, $alias = null, $default = false) {
   ...
   * @return Console
   */
  function addCommandsFromDir($dir, $namespace = '', $includeFiles = false) {
    foreach (new DirectoryIterator($dir) as $file) {
      $filename = $file->getFilename();
      if (
        $file->isDir() || substr($filename, 0, 1) === '.' || strlen($filename) <= 11
   ...
   * @return Console
   */
  function setDefaultCommand($name = null) {
   ...
   * @return Console
   */
  function setSingleCommand($singleCommand) {
   ...
   * @return mixed
   */
  function execute($command = null, array $args = array(), array $options = array()) {
